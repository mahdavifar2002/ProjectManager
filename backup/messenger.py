...
class MessengerUI(QWidget):
    def __init__(self, user: User):
        ...
        # Define our widgets
        self.usersVerticalLayout: QVBoxLayout = self.findChild(QVBoxLayout, "usersVerticalLayout")
        ...
        self.messagesScrollArea: QScrollArea = self.findChild(QScrollArea, "messagesScrollArea")
        self.scrollAreaWidgetContents: QWidget = self.findChild(QWidget, "scrollAreaWidgetContents")
        ...
        # self.messagesVerticalLayout: QVBoxLayout = self.findChild(QVBoxLayout, "messagesVerticalLayout")
        # self.messagesGridLayout: QGridLayout = self.findChild(QGridLayout, "messagesGridLayout")
    ...
    def load_messages(self):
        ...
        for message in messages:
            ...
            # textBrowser.setHtml("<font color='Aqua'>" + message.text)
            textBrowser.setFont(QFont("Vazirmatn"))
            textBrowser.setFixedHeight(int(textBrowser.document().size().height()))
            # textBrowser.setFixedHeight(100)
            print("size:", int(textBrowser.document().size().height()))
            self.gridLayout.addWidget(textBrowser)

        self.messagesScrollArea.verticalScrollBar().setSliderPosition(self.messagesScrollArea.verticalScrollBar().maximum())
